...
The five verbs an agent needs, which the whole design serves: enumerate (manifest), render (state-addressable in-app route), perceive (Playwright against stable URLs plus `data-bench-*` attributes), diff (import graph scopes what to re-screenshot), author (fixtures and notes as JSON).

- Repo: `github.com/andresCamp/bench` (Andrés's personal account, not onc9-systems)
- Local: `~/onc9-systems/bench`
- Run: `pnpm install && pnpm dev` → app at `localhost:5199`, bench at `/__bench`
- Stack: Solid + Vite 6 + Tailwind v4 + lucide-solid (only icon dep)

## Architecture map

- `tooling/bench-plugin.ts`: the substance. TS AST extraction (components, props, JSDoc), usage scan (app + intra-library tagged `internal`), manifest generation on every source change, and the notes HTTP API as Vite dev middleware. Also `handleHotUpdate` suppression for `.bench/` (see gotchas).
- `src/bench/data.ts`: client contract. Registry (recursive glob, multi-export, compound resolution), fixture types with `$component` refs, `coerceProps`, `stateUrl`, notes API client, `selectorWithin`.
- `src/bench/BenchIndex.tsx`: specimen-sheet library (no cards; live components on hairline rows; per-state comment count badges).
- `src/bench/BenchComponent.tsx`: the canvas page. Pan/zoom world, tick-tape zoom rail, glass chrome, knobs panel, notes layer (pins, regions, threads).
- `.bench/fixtures/*.json`: the only quasi-authored artifact; named states as data.
- `.bench/notes/*.json`: human+agent feedback, committed to git deliberately (notes travel with branches; resolution rides fix commits).
- `.bench/manifest.json`: derived, gitignored, never authored.

## The contract (this is the primitive; guard it)

1. **Manifest**: components with `name`, `slug`, `file`, `props` (kind: enum/boolean/string/number/children/unsupported, with `options`, `numeric`, JSDoc `description`), `usages` (file:line, `internal` flag), `target` for compounds, top-level `warnings`. Served at `GET /__bench/api/manifest`.
2. **State URL grammar**: `/__bench/<slug>?prop=value&...`. Every knob configuration is addressable. Composed children serialize as JSON strings in the query.
...
4. **Notes**: `{ id, component, stateUrl, selector, coords, rect?, text, author, status, created, replies[] }`. Coords/rect are stage-relative fractions and may exceed 0..1 (canvas notes). API: GET/POST `/__bench/api/notes/<slug>`, POST `.../move`, `.../resolve`, `.../reply`.
...
- **Vite full-reloads on writes to files outside the module graph.** Every note write reloaded the page until `handleHotUpdate` returns `[]` for `.bench/` paths. Any port to another bundler needs the equivalent.
...
- **Layout shift causes hover flicker.** The zoom percentage/reset swap needs fixed width (see `~/.claude/rules/design.md`; it applies to bench chrome too).
...
- **work**: the agent protocol above, as the default mode when bench is already installed. Bench is the unit test; real app flows via Playwright remain the integration test. Bench must never hijack flow-level verification (Andrés was explicit).
...
Framework portability: the contract above is framework-neutral. Extraction works for React TSX as-is; Svelte/Vue need their own extractors (compiler APIs). The bench UI must be written per host framework; the registry/Dynamic glue is ~40 lines per framework. Non-Vite hosts swap the plugin for equivalent route handlers.
