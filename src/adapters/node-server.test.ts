...
import { createBenchServer, type BenchServer } from "./node-server"
...
let root: string
let bench: BenchServer
let base: string

beforeAll(async () => {
  root = mkdtempSync(path.join(tmpdir(), "bench-server-"))
  ...
  mkdirSync(path.join(root, ".bench/fixtures"), { recursive: true })
  writeFileSync(
    path.join(root, ".bench/fixtures/button.json"),
    `{ "states": { "default": { "variant": "a" } } }`
  )
  bench = createBenchServer({ root })
  const port = await bench.listen()
...
describe("node-server driving adapter", () => {
  it("serves a fresh manifest with no watcher (regenerate-on-read)", async () => {
    const manifest = await fetch(`${base}/__bench/api/manifest`).then((r) => r.json())
    ...
    const after = await fetch(`${base}/__bench/api/manifest`).then((r) => r.json())
  ...
  it("serves fixtures and the full note lifecycle over HTTP", async () => {
    const fixture = await fetch(`${base}/__bench/api/fixtures/button`).then((r) => r.json())
    expect(fixture.states.default.variant).toBe("a")

    const created = await fetch(`${base}/__bench/api/notes/button`, {
      method: "POST",
    ...
    await fetch(`${base}/__bench/api/notes/button/reply`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: created.id, text: "ack", author: "agent" }),
    })
    const resolved = await fetch(`${base}/__bench/api/notes/button/resolve`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("doctor answers and non-API paths get a clear 404", async () => {
    const doctor = await fetch(`${base}/__bench/api/doctor`).then((r) => r.json())
