...
1. **Detect** the host: bundler (Vite? else the standalone server + proxy), framework (which Mounter it needs; Solid and React exist), component dir, and whether prop types are imported/intersections (turn on `typecheck`). Read `references/install-playbook.md` and follow the stack recipe. If the framework has no Mounter (Svelte, Vue), say so honestly and stop; writing one is lab work against `adapters/mount/contract.test.ts`, not install-time improvisation. The overlay needs no Mounter, so it can still be installed alone.
