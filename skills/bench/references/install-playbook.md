...
1. **Copy the engine** to `tooling/bench/`:
   ...
   - `src/adapters/{node-fs.ts,http.ts,vite.ts}` -> `tooling/bench/`
   - Fix relative imports in the adapters: `../core/...` and `../ports` become `./core/...` and `./ports`.
2. **Copy the surface** to `src/bench/`:
   - Solid: `src/adapters/client.ts` + `src/adapters/solid/{data.ts,BenchIndex.tsx,BenchComponent.tsx}` -> `src/bench/`
   - React: `src/adapters/client.ts` + `src/adapters/react/{data.ts,BenchIndex.tsx,BenchComponent.tsx}` -> `src/bench/`
   - Fix data.ts imports: `../../core/...` -> `../../tooling/bench/core/...`; `../client` -> `./client`.
...
4. **Wire the plugin** in `vite.config.ts`:
   ...
   plugins: [/* framework plugin */, tailwindcss(), bench({ componentDir: "src/components", typecheck: true })]
   ```
...
5. **Mount routes** in the app entry (import the register module for its side effect first):
   - Solid: `<Route path="/__bench" component={BenchIndex} />` + `<Route path="/__bench/:slug" component={BenchComponent} />`
   - React: `<Route path="/__bench" element={<BenchIndex />} />` + `<Route path="/__bench/:slug" element={<BenchComponent />} />`
...
9. **Verify**: dev server up; `curl /__bench/api/doctor` passes; Playwright renders `/__bench`; create + resolve a scratch note via the API; delete the scratch file.
...
2. Run it alongside dev (a `bench:api` script): 
   ```ts
   import { createBenchServer } from "./tooling/bench/node-server"
   const { listen } = createBenchServer({ root: process.cwd(), port: 5199, typecheck: true })
   listen()
   ```
   No watcher needed: manifest and doctor regenerate on every read.
3. Proxy `/__bench/api/*` to it:
   - Next (`next.config`): `rewrites: [{ source: "/__bench/api/:path*", destination: "http://localhost:5199/__bench/api/:path*" }]`
   - express: `http-proxy-middleware` on the same path.
4. Mount the React surface: Next app router gets a client page at `app/__bench/[[...slug]]/page.tsx` that renders BenchIndex/BenchComponent from the copied surface (the surface uses react-router hooks; in Next, wrap with a MemoryRouter synced to the pathname, or mount the surface in a tiny standalone Vite app if the host prefers isolation). This mapping has NOT been exercised against a real Next host yet; expect to promote fixes back.
5. `.bench/` scaffold, CLAUDE.md, and verification are identical to the Vite recipe.
...
Remove in reverse order: routes + register import, `src/bench/`, plugin/server wiring, `tooling/bench/`, `.gitignore` line, CLAUDE.md block (between its markers), orphaned deps (`lucide-*` if nothing else imports them). Then handle `.bench/`: fixtures are cheap to delete with the human's nod; **open notes are feedback: enumerate them with text + author, offer archive to `docs/bench-notes-archive/` vs delete, and default to archive.**
